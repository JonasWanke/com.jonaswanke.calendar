...
class CalendarView @JvmOverloads constructor(
    ...
    fun setEventsForWeek(week: Week, events: List<Event>) {
        val allEvents = this.events[week].orEmpty().filter { it.start !in week.millis } + events
        this.events[week] = allEvents
        weekViews[week]?.events = allEvents

        // Update future weeks
        var futureWeek = week.nextWeek
        while (this.events[futureWeek]?.any { it.start in week.millis } == true // Deprecated event has to be removed
